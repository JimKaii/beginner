...
# 從環境變數裏頭取得 access token與channel secret
...
def handle_message(event):
    ...
    if user_data.get("intent", "") != "訂高鐵" and user_message == "訂高鐵":
        ...
        今天是 {today}，但不代表出發日期,請把資料整理成python dictionary格式，例如：{standard_format}，
        不知道就填空字串，且回傳不包含其他內容。
        """
        ...
        if len(unfilled_slots) == 0:  # 全部填完
            ...
            create_drive()  # 目前只支持單人，driver是global的
